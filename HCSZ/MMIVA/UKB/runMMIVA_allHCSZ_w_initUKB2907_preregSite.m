function runMMIVA_allHCSZ_w_init
...
initpath=fullfile('/data/users2/eswar/temp/MISA_analysis/UKB/','results','hyb_','MIVA2step');
...
oSMRIdata = [FB1.SMRIdata(:,keepID_FBall(FBfinalloc))  CB1.SMRIdata(:,keepID_CBall(CBfinalloc)) MP1.SMRIdata(:,keepID_MPall(MPfinalloc)) BS1.SMRIdata(:,keepID_BSall(BSfinalloc)) ];
oALFFdata = [FB2.ALFFdata(:,keepID_FBall(FBfinalloc))  CB2.ALFFdata(:,keepID_CBall(CBfinalloc)) MP2.ALFFdata(:,keepID_MPall(MPfinalloc)) BS2.ALFFdata(:,keepID_BSall(BSfinalloc)) ];
oDMRIdata = [FB3.DMRIdata(:,keepID_FBall(FBfinalloc))  CB3.DMRIdata(:,keepID_CBall(CBfinalloc)) MP3.DMRIdata(:,keepID_MPall(MPfinalloc)) BS3.DMRIdata(:,keepID_BSall(BSfinalloc)) ];


siteREG= [FBDEMO.demo_out(FBfinalind,4);[ zeros(length(CBfinalind),1)] ;  MPDEMO.demo_out(MPfinalind,2)+3;BSDEMO.demo_out(BSfinalind, 3)];
ddd=dummyvar(siteREG+1);
dd2=ddd(:,sum(ddd)>0);
%uq=unique(siteREG);for cc=1:length(uq),nq(cc) = sum(siteREG==uq(cc));end

Xs = [ones(length(siteREG),1) dd2(:,2:end)];
bh1 = Xs\oSMRIdata';
SMRIdata = (oSMRIdata' - Xs*bh1)';
bh2 = Xs\oALFFdata';
ALFFdata = (oALFFdata' - Xs*bh2)';
bh3 = Xs\oDMRIdata';
DMRIdata = (oDMRIdata' - Xs*bh3)';
...
ful30 = load(fullfile(initpath,'UKBMRI_MMIVAfull_w0GICA_C030_results_goodSub2907_new_P000.mat'));
Winit = ful30.W;
clear ful30;
mmiva1 = setup_MISA_MMIVA1(M,X,Winit,'kotz');
[wout,fval,exitflag,output] = run_MISA(mmiva1,Winit);
...
%fname = sprintf('HCSZMRI_MMIVAreduced_wUKBinit_C%03d_results_new_P%03d',step3_reduced.C(1),P);
...
fname = sprintf('HCSZMRI_MMIVAfull_w0UKBinit2907_C%03d_results_new_P%03d_preregSite',step3_full.C(1),P);
save(fullfile(outpath,[fname '.mat']), ...
        'W','A_tr','A_pinv','A_ls1','icasig','step3_full','vn_old_mean','vn_old_std','old_mean','outpath','-v7.3')
